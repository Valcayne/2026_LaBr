...
Double_t fitFunction_v2(Double_t *x, Double_t *par) {
  ...
  //return (par[0]*exp(-0.5*(pow((x[0]-par[1])/par[2],2)))+par[3]*exp(-0.5*(pow((x[0]-par[1]*((4438.-511.)/4438.))/par[2],2))));
  return (par[0]*exp(-0.5*(pow((x[0]-par[1])/par[2],2)))+par[3]*exp(-0.5*(pow((x[0]-par[1]*((4438.-511.)/4438.))/par[2],2)))+par[4]*exp(-0.5*(pow((x[0]-par[1]*((4438.-1022.)/4438.))/par[2],2))));
}

Double_t fitFunction_v3(Double_t *x, Double_t *par) {
  //Gaussian CMC
  //return (par[0]*exp(-0.5*(pow((x[0]-par[1])/par[2],2)))+par[3]*exp(-0.5*(pow((x[0]-par[1]*((4438.-511.)/4438.))/par[2],2))));
  return (par[0]*exp(-0.5*(pow((x[0]-par[1])/par[2],2)))+par[3]*exp(-0.5*(pow((x[0]-par[1]*((6130.-511.)/6130.))/par[2],2)))+par[4]*exp(-0.5*(pow((x[0]-par[1]*((6130.-1022.)/6130.))/par[2],2))));
}

Double_t fitFunction2(Double_t *x, Double_t *par) {
  //Gaussian + straight line
  return (par[0]+par[1]*x[0])+(par[2]*exp(-0.5*(pow((x[0]-par[3])/par[4],2))));
}

Double_t fitFunction2_v2(Double_t *x, Double_t *par) {
  //2 Gaussians + straight line - Am/Be source
  //return (par[0]+par[1]*x[0])+(par[2]*exp(-0.5*(pow((x[0]-par[3])/par[4],2))))+(par[5]*exp(-0.5*(pow((x[0]-par[3]*((4438.-511.)/4438.))/par[4],2))));
  return (par[0]+par[1]*x[0])+(par[2]*exp(-0.5*(pow((x[0]-par[3])/par[4],2))))+(par[5]*exp(-0.5*(pow((x[0]-par[3]*((4438.-511.)/4438.))/par[4],2))))+(par[6]*exp(-0.5*(pow((x[0]-par[3]*((4438.-1022.)/4438.))/par[4],2))));
}

Double_t fitFunction2_v3(Double_t *x, Double_t *par) {
  //2 Gaussians + straight line - Cm/C source
  //return (par[0]+par[1]*x[0])+(par[2]*exp(-0.5*(pow((x[0]-par[3])/par[4],2))))+(par[5]*exp(-0.5*(pow((x[0]-par[3]*((4438.-511.)/4438.))/par[4],2))));
  return (par[0]+par[1]*x[0])+(par[2]*exp(-0.5*(pow((x[0]-par[3])/par[4],2))))+(par[5]*exp(-0.5*(pow((x[0]-par[3]*((6130.-511.)/6130.))/par[4],2))))+(par[6]*exp(-0.5*(pow((x[0]-par[3]*((6130.-1022.)/6130.))/par[4],2))));
}
...
TACCalibrator::~TACCalibrator(){
  for(int i=0;i<NDETECTORS+1;i++){
    ...
    if(b[i]!=0){delete b[i];}
    for(int j=0;j<MAXNLOCALMAX;j++){
      if(FitFun[i][j]){delete FitFun[i][j];}
      if(FitFun2[i][j]){delete FitFun2[i][j];}
    }
  }
}
...
int TACCalibrator::FindPeaks(int CrNumber){
  ...
  if(FWHM_ch<5){FWHM_ch=5;}
  int halfFWHM_ch=FWHM_ch/2;
  for(int i=(halfFWHM_ch+1);i<=d[CrNumber]->GetNbinsX()-(halfFWHM_ch+1);i++){
  ...
  for(int i=0;i<MAXNLOCALMAX;i++){
    if(FitFun[CrNumber][i]!=0){delete FitFun[CrNumber][i]; FitFun[CrNumber][i]=0;}
  }
  for(int i=0;i<MAXNLOCALMAX;i++){
    if(FitFun2[CrNumber][i]!=0){delete FitFun2[CrNumber][i]; FitFun2[CrNumber][i]=0;}
  }
  NFitFun[CrNumber]=0;
  ...
  if(nlocalmax>0){
    ...
    for(int i=0;i<nlocalmax;i++){
...
#ifdef AMBESOURCE
      FitFun[CrNumber][i]=new TF1(name,fitFunction_v2,TMath::Max(thelocalmax-FWHM,(double)HISTOLOWLIM)
				  ,TMath::Min(thelocalmax+FWHM,(double)HISTOHILIM),5);
#endif
#ifdef CMCSOURCE
      FitFun[CrNumber][i]=new TF1(name,fitFunction_v3,TMath::Max(thelocalmax-FWHM,(double)HISTOLOWLIM)
				  ,TMath::Min(thelocalmax+FWHM,(double)HISTOHILIM),5);
#endif 
#ifdef CSYSOURCE
      
      FitFun[CrNumber][i]=new TF1(name,fitFunction,TMath::Max(thelocalmax-FWHM,(double)HISTOLOWLIM)
      ...
      {
        FitFun[CrNumber][i]->SetParameters(const1[kkk], cent[kkk], sig[kkk], const2[kkk], const3[kkk]);
      ...
      FitFun[CrNumber][i]->SetParameters(const1[i_bestchi], cent[i_bestchi], sig[i_bestchi], const2[i_bestchi], const3[i_bestchi]);
      d[CrNumber]->Fit(FitFun[CrNumber][i], "NRQ");
#endif
#ifdef CMCSOURCE
      int const NTIMES = 2;
...
--------------------
      ...
      //--------------------------------------------------------------------
      //Second fit:
      constant=FitFun[CrNumber][i]->GetParameter(0);
...
r_min=center*((6130.-511.)/6130.)-4*sigma;
	...
	if(h[CrNumber]->GetBinCenter(j)>r_min){
	  y_rmin=(h[CrNumber]->GetBinContent(j-1)+h[CrNumber]->GetBinContent(j)+h[CrNumber]->GetBinContent(j+1))/3.; break;
	}
      }
      for(int j=1;j<=h[CrNumber]->GetNbinsX();j++){
	if(h[CrNumber]->GetBinCenter(j)>r_max){
	  y_rmax=(h[CrNumber]->GetBinContent(j-1)+h[CrNumber]->GetBinContent(j)+h[CrNumber]->GetBinContent(j+1))/3.; break;
	}
      }
      r_m=(y_rmax-y_rmin)/(r_max-r_min);
      r_b=y_rmax-r_m*r_max;
#ifdef AMBESOURCE
      FitFun2[CrNumber][i]->SetParameters(r_b,r_m,constant,center,sigma,constant,FitFun[CrNumber][i]->GetParameter(3),FitFun[CrNumber][i]->GetParameter(4));
#endif
#ifdef CMCSOURCE
      FitFun2[CrNumber][i]->SetParameters(r_b,r_m,constant,center,sigma,constant,FitFun[CrNumber][i]->GetParameter(3),FitFun[CrNumber][i]->GetParameter(4));
#endif
...
int TACCalibrator::SaveValues(string filename)
{
  ...
  for (int i = 0; i < NDETECTORS; i++)
  {
    ...
    for (int j = 0; j < NFitFun[i + 1]; j++)
    {
      ...
      //--------------------------
#ifdef AMBESOURCE
      if (FitFun2[i + 1][j]->GetParameter(2) * 5 < FitFun2[i + 1][j]->GetParameter(5) && j == NFitFun[i + 1] - 1)
      {
        cout << " **** WARINIG: in crystal " << i + 1 << " area(4438-511)=" << FitFun2[i + 1][j]->GetParameter(5) << " and area(4438)=" << FitFun2[i + 1][j]->GetParameter(2) << " ****" << endl;
      }
#endif
